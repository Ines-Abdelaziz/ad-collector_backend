...
def store_Data_To_DataBase(request):
    ...
    try:
        ...
        if json_data["connected"] == False :
            ...
            user_id2 = sha512(str(json_data["user"]["id"]).encode('utf-8')).hexdigest()
            user_id = sha512(str(json_data["user"]["id"]).encode('utf-8')).hexdigest()
            print('user igggggggggg',user_id,json_data["user"]["id"],user_id2)           
 #user_name = json_data["user"]["name"]
             #user_email = json_data["user"]["email"]
        #user_country = json_data["country"]
        if user.objects.filter(id=user_id).exists():
            print('I found the user yes no worries')
            u = user.objects.filter(id=user_id).first() 
            consent= u.consent
            print('Here is the consent we found ',user_id)
            if consent :
		print('I have the consent tooooooooooooo')
                response['saved'] = True
...
def data_general_stat(user_id):
    ...
    #advertiser
        ...
        advertiserVideos = list(videoAd.objects.filter(users=user_id).exclude(ad_page_link="").values('ad_page_link').annotate(num_ads=Count('id')).order_by(
            "-num_ads"))
        #print('okey ca donne ca',advertiserVideos)
        advertiserFloats = list(FloatingAd.objects.filter(users=user_id).exclude(link="").values('link').annotate(num_ads=Count('id')).order_by(
            "-num_ads"))
       # print('okey ca donne ca',advertiserFloats) 
        dic = OrderedDict()
        for advertiserV in advertiserVideos:
            dic[advertiserV["ad_page_link"]] = advertiserV["num_ads"]
        for advertiserF in advertiserFloats:
            if advertiserF["link"] in dic :
                dic[advertiserF["link"]] += advertiserF["num_ads"]
            else : dic[advertiserF["link"]] = advertiserF["num_ads"]
        print('okey ca donne ca',dic) 
...
@csrf_exempt
def isConsent(request):
    print('here we go')
    response = {}
    body = request.body.decode('utf-8')
    json_data = json.loads(body)
    print(json_data)
    user_id = sha512(str(json_data['user_id']).encode('utf-8')).hexdigest()
    if user.objects.filter(id=user_id).exists():
	exist = user.objects.filter(id=user_id).first() 
        consent= exist.consent
        print('We found it',user_id)
        ...
        u.save()
        print('We not found and create it')
        consent = False
    response["status"] = 'success'
...
def acceptConsent(request):
    ...
    user_id = sha512(str(json_data['user_id']).encode('utf-8')).hexdigest()
    if user.objects.filter(id=user_id).exists():
        exist = user.objects.filter(id=user_id).update(consent=True) 
        response["consent"] = True
        print('We found and changed it')
  
    else:
        print('We not found and not changed it')
...
def removeConsent(request):
    ...
    user_id = sha512(str(json_data['user_id']).encode('utf-8')).hexdigest()
    if user.objects.filter(id=user_id).exists():
        user.objects.filter(id=user_id).update(consent=False) 
        response["consent"] = True
